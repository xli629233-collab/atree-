...
// Hook to generate Polka Dot Texture for Frames
...
export const Tree: React.FC<TreeProps> = ({ photos, onPhotoClick, isExploded, isTwinkling, gestureRotation, foliageColor = '#064e3b', treeStyle, shape }) => {
  ...
  const sparkleColor = useMemo(() => {
      if (foliageColor === 'rainbow') return '#ffffff';
      if (foliageColor === '#064e3b') return '#FFD700'; // Green tree gets gold sparkles
      return foliageColor; // Gold gets Gold, Silver gets Silver
  }, [foliageColor]);
    ...
    <group ref={groupRef}>
      ...
      <group visible={!isExploded}>
        ...
        <Sparkles 
            ...
            noise={0.5}
        />
        <pointLight position={[0,0,0]} intensity={isTwinkling ? 3 : 1} distance={15} color={sparkleColor} />
      </group>
